...
### --- Read in multilevel data frame *pfx_16* --- ###
load(url("http://aaronbaggett.com/data/pfx_16.rda"))
...
m0 <- glm(u_test ~ 1, data = pfx_16, family = binomial)
...
m1 <- glmer(u_test ~ 1 + (1 | umpire), data = pfx_16,
...
glm_model <- glmer(u_test ~ factor(zone_reg)*factor(bs_count) + 
...
m5 <- glmer(u_test ~ scale(yr_exp) + (1 | umpire), data = pfx_16, 
  family = binomial, nAGQ = 0)
...
m6 <- glmer(u_test ~ factor(zone_reg)*scale(yr_exp) + 
  ...
  (1 | umpire), data = pfx_16, family = binomial, nAGQ = 0)
